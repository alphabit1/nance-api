...
const schema = {
  depth: ({ symbol, updateSpeed = 1000 }) =>
    `${symbol.toLowerCase()}@depth@${updateSpeed}ms`, // updateSpeed: 100 or 1000
  depthLevel: ({ symbol, levels = 100, updateSpeed = 1000 }) =>
    `${symbol.toLowerCase()}@depth${levels}@${updateSpeed}ms`,
  kline: ({ symbol, interval = "1h" }) =>
...
const api = Object.keys(schema).reduce((result, item) => {
  result[item] = (params, callback) =>
    setupWebSocket({ path: schema[item](params), ...params }, callback);
  return result;
}, {});
...
const closeSocketApi = Object.keys(schema).reduce((result, item) => {
  result[item] = (params) => {
    return params && params.uniqueID
      ? closeSocket(params.uniqueID)
      : closeSocket(schema[item](params));
  };
  return result;
}, {});
