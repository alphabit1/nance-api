...
const api = Object.keys(schema).reduce((result, item) => {
  result[item] = (payload) =>
    item == "createListenKey"
      ? createListenKey(schema[item], payload)
      : request(schema[item], payload);
  return result;
}, {});
...
const keepAlive = ({ auth, params }) => {
  ...
  keepAliveInterval = setInterval(() => {
    api
      .keepAliveListenKey({ auth, params })
...
const userData = ({ auth, uniqueID, proxy }, callback) => {
  api.createListenKey({ auth, proxy }).then((res) => {
