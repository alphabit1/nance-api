...
const requestPrivateAPI = (
  ...
  const queryString = qs.stringify(
    Object.assign(params, defaultParams, { timestamp })
  );
...
const requestPublicAPI = (data, { proxy = {}, ...params }, event) => {
  return axiosWithCache
    .get(proxyWrapper(url + data.url, proxy), { params })
    .then((res) => rename(res.data, event));
};

const api = Object.keys(schema).reduce((result, item) => {
  result[item] = (payload) =>
    schema[item].private
      ? requestPrivateAPI(schema[item], payload, item)
      : requestPublicAPI(schema[item], payload, item);
  return result;
}, {});
